defmodule MeshxRpc do
  ...
  RPC client can be accessed directly using `MeshxRpc.Client.Pool` or using wrapper module `MeshxRpc.Client`. Similarly RPC server is available through `MeshxRpc.Server` or `MeshxRpc.Server.Pool` modules.

  Configuration options common to RPC client and server:
  ...
  Events generated by `MeshxRpc`:
    * `:init` - emitted only by server when server worker cannot establish socket connection with user provided `address` which should be terminated with transport solution (eg. service mesh sidecar proxy),
  ...
  * `:result` - execution result. If request execution was successful `:result` is set to atom `:ok`. If execution failed full error is reported,
  ...
  * `:state` - worker `:gen_statem` last state, e.g. `:reply`.
  ...
  Errors are logged with `:error` Logger level, all other event are logged with `:debug` level.

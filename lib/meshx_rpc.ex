defmodule MeshxRpc do
  ...
  RPC client and server modules provide child specifications which should be used with user supervisors as shown on above examples. RPC client `child_spec` can be created directly accessing `MeshxRpc.Client.Pool.child_spec/2` or by using wrapper module `MeshxRpc.Client`. Similarly RPC server is available through `MeshxRpc.Server.Pool.child_spec/2` or `MeshxRpc.Server` modules.
  ...
  Events generated by `MeshxRpc`:
    * `:init` - emitted by server when server worker cannot establish socket connection with user provided `address` terminated with transport solution (eg. service mesh sidecar proxy),
  ...
  ### Telemetry metadata
  * `:address` - connection address, eg. `{:tcp, {127, 0, 0, 1}, 1024}`,
  * `:fun_name` - request function name, eg. `:echo`,
  * `:fun_req` - request function type, can be `:call` or `:cast`,
  * `:hsk_ref` - handshake reference, `integer()`,
  * `:id` - RPC server or client id, eg. `Example2.Client`,
  ...
  * `:result` - execution result. If request execution was successful `:result` is set to atom `:ok`, real execution results are not emitted. If execution failed full error is reported,
  ...
  * `:state` - worker `:gen_statem` last state, e.g. `:reply`.
  ...
  First argument should correspond to `:telemetry_prefix` configuration option described earlier. Second argument is telemetry [handler id](https://hexdocs.pm/telemetry/telemetry.html#type-handler_id). If handler id is undefined it will be assigned value equal to second prefix list argument.
